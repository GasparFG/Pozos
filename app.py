...
def _normalize_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for key in ["date", "fecha", "fechas"]:
        if key in cols_low:
            rename_map[cols_low[key]] = "date"
            break
    for key in ["well", "pozo", "wells", "pozos"]:
        if key in cols_low:
            rename_map[cols_low[key]] = "well"
            break
    for key in ["oil bbl", "oil", "petróleo", "petroleo"]:
        if key in cols_low:
            rename_map[cols_low[key]] = "Oil"
            break
    for key in ["water bbl", "water", "agua"]:
        if key in cols_low:
            rename_map[cols_low[key]] = "Water"
            break
    for key in ["gas mcf", "gas"]:
        if key in cols_low:
            rename_map[cols_low[key]] = "Gas"
            break
...
def calcular_metricas(df, var, unidad_q, unidad_v):
    ...
    data = {
        ...
        "Valor": [
            ...
            fmt_num(total_meses),
            fmt_num(prod_acum),
            fmt_fecha(fecha_inicio),
            fmt_fecha(fecha_final)
        ],
        "Unidad": [
            unidad_q,
            unidad_q,
...
if st.session_state.confirmed:
    ...
    pozos_sel = st.multiselect(
        "Pozos", options=pozos, default=st.session_state["pozos_sel"])
    ...
    if st.button("Seleccionar/Quitar todos los pozos"):
        if set(st.session_state["pozos_sel"]) == set(pozos):
            ...
            st.session_state["pozos_sel"] = pozos
        st.rerun()
    df_filt_base = df_month[df_month["well"].isin(
        st.session_state["pozos_sel"])]
    ...
    min_ts, max_ts = df_filt_base["period"].min().to_pydatetime(
    ), df_filt_base["period"].max().to_pydatetime()
    ...
    if st.button("Reiniciar fecha"):
        st.session_state["fecha_range"] = (min_ts, max_ts)
        st.rerun()
    df_filt = df_filt_base[(df_filt_base["period"] >= start_dt) & (
        df_filt_base["period"] <= end_dt)]
    # --- Pestañas ---
    ...
    with tabs[0]:
        ...
        for var in ["Oil", "Gas", "Water"]:
            if var in df_filt.columns:
                ...
                df_tmp["V_acum"] = (
                    df_tmp[var] * df_tmp["period"].dt.days_in_month).cumsum()
                df_tmp["Q"] = df_tmp[var]
                df_tmp["fluido"] = var
                df_total[var] = df_tmp
        ...
        chart_total = alt.layer(
            chart_q, chart_v).resolve_scale(y="independent")
        st.altair_chart(chart_total, use_container_width=True)
    # Oil, Gas, Water
    for i, var in enumerate(vars_ok, start=1):
        with tabs[i]:
            st.subheader(f"{var} por fecha y pozo")
            chart_type = st.radio("Tipo de gráfica", [
                                  "Líneas", "Área apilada (valores)"], horizontal=True, key=f"chart_type_{var}")
            if chart_type == "Líneas":
                chart_pozos = (
                    ...
                    .encode(
                        ...
                        color=alt.Color("well:N", title="Pozo", legend=alt.Legend(
                            orient="bottom", columns=5)),
                        tooltip=["period:T", "well:N", alt.Tooltip(
                            f"{var}:Q", format=",.2f")]
                    )
                )
                df_avg = df_filt.groupby("period", as_index=False)[var].mean()
                df_avg["well"] = "PROMEDIO"
                chart_avg = alt.Chart(df_avg).mark_line(strokeDash=[
                    4, 2], color="black", strokeWidth=2).encode(x="period:T", y=alt.Y(f"{var}:Q"))
            ...
            met = calcular_metricas(df_filt, var, unidad_q, unidad_v)
            st.dataframe(met)
            # Total por fluido
            st.subheader(f"TOTAL {var.upper()}")
            df_var = df_filt.groupby("period", as_index=False)[var].sum()
            df_var["V_acum"] = (
                df_var[var] * df_var["period"].dt.days_in_month).cumsum()
            df_var["Q"] = df_var[var]
            base = alt.Chart(df_var).encode(x="period:T")
            line_q = base.mark_line(color="blue").encode(
                y=alt.Y("Q:Q", title="Q"))
            line_v = base.mark_line(color="red", strokeDash=[4, 2]).encode(
                y=alt.Y("V_acum:Q", title="V acumulado"))
            chart_total_var = alt.layer(
                line_q, line_v).resolve_scale(y="independent")
    ...
    with tabs[-1]:
        ...
        for i, pozo in enumerate(sorted(df_filt["well"].unique())):
            ...
            met = calcular_metricas(df_p, vars_ok[0], unidad_q, unidad_v)
            with cols[i % 3]:
                st.markdown(f"### {pozo}")
                st.table(met)
