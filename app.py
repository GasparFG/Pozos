...
def calcular_metricas(df, var, unidad):
    ...
    coef_cambio = (serie.iloc[-1] - serie.iloc[0]) / serie.iloc[0] if serie.iloc[0] != 0 else np.nan
    ...
    data = {
        "Métrica": ["Media", "Máximo", "Fecha máximo", "Mínimo", "Fecha mínimo", "Coeficiente de cambio"],
        "Valor": [
            fmt_num(promedio),
            fmt_num(max_val),
            fmt_fecha(max_fecha),
...
if st.session_state.confirmed:
    ...
    # Si está fuera de rango, reiniciamos
    ...
    # --- Pestañas ---
    vars_ok = [c for c in ["Oil", "Water", "Gas"] if c in df_filt.columns]
    tabs = st.tabs(vars_ok + ["TOTAL", "Pozos"])

    for tab, var in zip(tabs, vars_ok):
        with tab:
            st.subheader(f"{var} por fecha y pozo")

            chart_type = st.radio(
                "Tipo de gráfica",
                ["Líneas", "Área apilada (valores)"],
                horizontal=True,
                key=f"chart_type_{var}"
            )
            ...
            if chart_type == "Líneas":
                chart_pozos = (
                    ...
                    .encode(
                        ...
                        color=alt.Color("well:N", title="Pozo",
                ...
                chart_avg = (
                    alt.Chart(df_avg)
                    .mark_line(strokeDash=[4,2], color="black", strokeWidth=2)
                    .encode(x="period:T", y=alt.Y(f"{var}:Q"))
                )
                chart = alt.layer(chart_pozos, chart_avg)

            elif chart_type == "Área apilada (valores)":
                chart = (
                    alt.Chart(df_filt)
            ...
            chart = chart.properties(height=350).interactive()
            st.altair_chart(chart, use_container_width=True)

            unidad = "BPD" if var in ["Oil","Water"] else "MCFD"
            st.subheader("Métricas descriptivas")
            met = calcular_metricas(df_filt, var, unidad)
            st.dataframe(met)

    # --- Pestaña TOTAL ---
    with tabs[len(vars_ok)]:
        st.subheader("TOTAL (V acumulado vs Q)")
        df_total = df_filt.groupby("period", as_index=False)[["Oil","Water","Gas"]].sum()
        df_total["V_total"] = df_total[["Oil","Water","Gas"]].sum(axis=1) * df_total["period"].dt.days_in_month
        df_total["Q_total"] = df_total["V_total"] / df_total["period"].dt.days_in_month
        df_total["V_acum"] = df_total["V_total"].cumsum()

        base = alt.Chart(df_total).encode(x="period:T")

        line_v = base.mark_line(color="red").encode(
            y=alt.Y("V_acum:Q", title="V acumulado", axis=alt.Axis(titleColor="red"))
        )

        line_q = base.mark_line(color="blue").encode(
            y=alt.Y("Q_total:Q", title="Q", axis=alt.Axis(titleColor="blue"))
        )

        chart = alt.layer(line_v, line_q).resolve_scale(y="independent")
        st.altair_chart(chart, use_container_width=True)

    # --- Pestaña Pozos ---
    with tabs[len(vars_ok)+1]:
        st.subheader("Métricas por pozo (Q)")
        unidad = "BPD" if "Oil" in vars_ok else "MCFD"
        cols = st.columns(3)
        for i, pozo in enumerate(sorted(df_filt["well"].unique())):
            df_p = df_filt[df_filt["well"] == pozo]
            met = calcular_metricas(df_p, vars_ok[0], unidad)  # por defecto usando la primera variable disponible
            with cols[i % 3]:
                st.markdown(f"### {pozo}")
                st.table(met)
